...
var button_clear_all = document.getElementById('button-clear-all')
...
// тут output_text = document.getElementById('log')
// input = document.getElementById('input')
//     <img id="input" src="http://web-vin/images/addempty.png">
// input_overlay = document.getElementById('input-overlay')

function setUp(){
    input_overlay.width = input.naturalWidth
    input_overlay.height = input.naturalHeight

    output_text.style.height = (input.height +40)+ 'px'
	if (button_clear_all) {
        button_clear_all.style.display = 'none'
    }
}
...
function progressUpdate(packet){
	...
	if(log.firstChild && log.firstChild.status === packet.status){
		...
		if(packet.status == 'done'){
            ...
            button_clear_all.style.display = 'block';
		}
...
function ClearLog() {
    ...
    var log = document.getElementById('text-log');
